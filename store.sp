...
new String:Msg_[MAXPLAYERS+1][128];
...
new bool:g_bSearchMode[MAXPLAYERS+1];
...
public OnPluginStart()
{
	...
	RegConsoleCmd("say", Say_Shop, "Opens the tetris minigame.");
...
public CategoryHandler_StoreAdmin(Handle:topmenu, TopMenuAction:action, TopMenuObject:object_id, param, String:buffer[], maxlength)
{
	if (action == TopMenuAction_DisplayTitle || action == TopMenuAction_DisplayOption)
		Format(buffer, maxlength, "스토어 어드민 메뉴");
}
...
//	{
//		Format(buffer, maxlength, "데이타 베이스 초기화(위험합니다!)");
//	}
//	else if (action == TopMenuAction_SelectOption)
//	{
//		g_iMenuNum[client] = 0;
//		Store_DisplayConfirmMenu(client, "당신은 데이타 베이스를 초기화하기 원합니까?\n※경고 모든 플레이어가 초기화 됩니다..", FakeMenuHandler_ResetDatabase, 0);
//	}
//}
//
...
public MenuHandler_ResetPlayer(Handle:menu, MenuAction:action, client, param2)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		else
		{
			...
			Format(STRING(m_szTitle), "당신은 %s님을 초기화하기 원합니까?", m_szName);
...
public AdminMenu_GiveCredits(Handle:topmenu, TopMenuAction:action, TopMenuObject:object_id, client, String:buffer[], maxlength)
{
	if (action == TopMenuAction_DisplayOption)
	{
		Format(buffer, maxlength, "플레이어에게 크레딧 주기");
	}
	else if (action == TopMenuAction_SelectOption)
	{
		g_iMenuNum[client] = 5;
		new Handle:m_hMenu = CreateMenu(MenuHandler_GiveCredits);
		SetMenuTitle(m_hMenu, "크레딧을 줄 플레이어를 선택하세요.");
		SetMenuExitBackButton(m_hMenu, true);
		LoopAuthorizedPlayers(i)
		{
			decl String:m_szName[64];
...
public MenuHandler_GiveCredits(Handle:menu, MenuAction:action, client, param2)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		SetMenuTitle(m_hMenu, "선택할 크레딧 금액\n%N - %d 크레딧", target, g_eClients[target][iCredits]);
		SetMenuExitBackButton(m_hMenu, true);
		AddMenuItem(m_hMenu, "-1000", "-1000 크레딧 주기");
		AddMenuItem(m_hMenu, "-100", "-100 크레딧 주기");
		AddMenuItem(m_hMenu, "-10", "-10 크레딧 주기");
		AddMenuItem(m_hMenu, "10", "10 크레딧 주기");
		AddMenuItem(m_hMenu, "100", "100 크레딧 주기");
		AddMenuItem(m_hMenu, "1000", "1000 크레딧 주기");
		AddMenuItem(m_hMenu, "10000", "1만 크레딧 주기");
		AddMenuItem(m_hMenu, "100000", "10만 크레딧 주기");
		AddMenuItem(m_hMenu, "1000000", "100만 크레딧 주기");
		DisplayMenu(m_hMenu, client, 0);
	}
	else if (action == MenuAction_Cancel && param2 == MenuCancel_ExitBack)
		RedisplayAdminMenu(g_hAdminMenu, client);
...
public AdminMenu_ViewInventory(Handle:topmenu, TopMenuAction:action, TopMenuObject:object_id, client, String:buffer[], maxlength)
{
	if (action == TopMenuAction_DisplayOption)
	{
		Format(buffer, maxlength, "플레이어 인벤토리 확인");
	}
	else if (action == TopMenuAction_SelectOption)
	{
		g_iMenuNum[client] = 4;
		new Handle:m_hMenu = CreateMenu(MenuHandler_ViewInventory);
		SetMenuTitle(m_hMenu, "인벤토리를 볼 플레이어를 선택하십시오");
		SetMenuExitBackButton(m_hMenu, true);
		LoopAuthorizedPlayers(i)
		{
			decl String:m_szName[64];
...
public OnClientPutInServer(client)
{
	...
	TfAtt_OnClientPutInServer(client);
	//Glow_OnClientPutInServer(client);
}
#endif
...
public Action:Say_Shop(client, args)
{
	new String:Msg[128];
	...
	if ('$'==Msg[1])
	{
		ReplaceString(Msg, 128, "\"", "");
		ReplaceString(Msg, 128, "$", "");
		Command_Store2(client, Msg);
	}
	else if (StrContains(Msg,"!스토어") != -1 )
	{
		ReplaceString(Msg, sizeof(Msg), "!스토어", "");
		Command_Store2(client, Msg);
	}
	else if (StrContains(Msg,"/스토어") != -1 )
	{
	...
	else if (StrContains(Msg,"!샵") != -1 )
	{
		ReplaceString(Msg, sizeof(Msg), "!샵", "");
		Command_Store2(client, Msg);
	}
	else if (StrContains(Msg,"/샵") != -1 )
	{
	...
	else if (StrContains(Msg,"!상점") != -1 )
	{
		ReplaceString(Msg, sizeof(Msg), "!상점", "");
		Command_Store2(client, Msg);
	}
	else if (StrContains(Msg,"/상점") != -1 )
	{
...
stock Command_Store2(client, char[] message)
{
	...
	//	return Plugin_Handled;
	//}
	//ReplaceString(message, 128, "\"", "");
	//ReplaceString(message, 128, " ", "");

	if(!(StrContains("", message, false) != -1)){
		g_bSearchMode[client]=true;
		Format(Msg_[client], 128, "%s", message);
	}
...
public Action:Command_Store(client, params)
{
	...
	char Msg[128];
	GetCmdArgString(Msg, sizeof(Msg));
	if(!(StrContains("", Msg, false) != -1)){
		g_bSearchMode[client]=true;
		Format(Msg_[client], 128, "%s", Msg);
	}
...
DisplayStoreMenu(client, parent=-1, last=-1)
{
	...
	for(new i=0;i<g_iItems;++i)
	{
		if(g_bSearchMode[client]){
			if(g_eItems[i][iPrice]>=0){
				if(!(StrContains(g_eItems[i][szName], Msg_[client], false) != -1)){
					continue;
				}
			}
			else if(g_eItems[i][iPrice]==-1)
			{
				Format(Msg2, sizeof(Msg2), "%s" ,g_eItems[i][szName]);
				bbb=true;
				continue;
			}
		}
		aaa++;
		if((g_bSearchMode[client]?true:g_eItems[i][iParent]==parent) && (g_eCvars[g_cvarShowVIP][aCache] == 0 && GetClientPrivilege(target, g_eItems[i][iFlagBits], m_iFlags) || g_eCvars[g_cvarShowVIP][aCache]))
		{
			...
			else
			{
				...
				if(Store_HasClientItem(target, i))
				{
					if(Store_IsEquipped(target, i)){
						if(g_bSearchMode[client])
							InsertMenuItemEx(m_hMenu, m_iPosition, ITEMDRAW_DEFAULT, m_szId, "[%s] %t", Msg2,"Item Equipped", g_eItems[i][szName]);
						else
							InsertMenuItemEx(m_hMenu, m_iPosition, ITEMDRAW_DEFAULT, m_szId, "%t", "Item Equipped", g_eItems[i][szName]);
					}
					else{
						if(g_bSearchMode[client])
							InsertMenuItemEx(m_hMenu, m_iPosition, ITEMDRAW_DEFAULT, m_szId, "[%s] %t", Msg2,"Item Bought", g_eItems[i][szName]);
						else
				...
				{				
					if(g_bSearchMode[client]){
						if(bbb){
							decl String:Msg3[128];
							Format(Msg3, sizeof(Msg3), "===%s===" ,Msg2);
							AddMenuItem(m_hMenu, "", Msg3,ITEMDRAW_DISABLED);
							bbb=false;
						}
					}
					new m_iStyle = ITEMDRAW_DEFAULT;
	...
	if(g_bSearchMode[client]){
		if(ccc)
			Format(title_,sizeof(title_),"%t (%s)\n%t\n\n======%t======", "Title Store",Msg_[client] ,"Title Credits", g_eClients[target][iCredits],"My Item");
		else
			Format(title_,sizeof(title_),"%t (%s)\n%t", "Title Store", Msg_[client],"Title Credits", g_eClients[target][iCredits]);
		if(aaa==0)
			Chat(client, "%t", "No Search Results",Msg_[client]);
		else
			Chat(client, "%t", "Search Results",Msg_[client],aaa);
	}
	SetMenuTitle(m_hMenu, title_);
	if(last == -1)
		DisplayMenu(m_hMenu, client, 0);
	else
