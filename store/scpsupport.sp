...
new String:g_szTagColors[STORE_MAX_ITEMS][32];
...
{	
	...
	Store_RegisterHandler("tagcolor", "color", SCPSupport_OnMappStart, SCPSupport_Reset, TagColors_Config, SCPSupport_Equip, SCPSupport_Remove, true);
...
public TagColors_Config(&Handle:kv, itemid)
{
	Store_SetDataIndex(itemid, g_iTagColors);
	KvGetString(kv, "color", g_szTagColors[g_iTagColors], sizeof(g_szTagColors[]));
	++g_iTagColors;
...
public Action:OnChatMessage(&client, Handle:recipients, String:name[], String:message[])
{
	...
	if(m_iEquippedTagColor >= 0)
	{
		new m_iTagColor = Store_GetDataIndex(m_iEquippedTagColor);
		strcopy(STRING(m_szTagColor), g_szNameColors[m_iTagColor]);
	}
	if(m_iEquippedNameColor >= 0)
	{
		...
		if (GetAdminFlag(GetUserAdmin(client), Admin_Root))
			Format(STRING(m_szName), "%s%s%s%s%s",m_szTagColor,m_szNameTag,"{green}★",m_szNameColor, playerName);
		else if (GetAdminFlag(GetUserAdmin(client), Admin_Kick))
			Format(STRING(m_szName), "%s%s%s%s%s",m_szTagColor,m_szNameTag,"{lime}▲",m_szNameColor, playerName);
		else
			Format(STRING(m_szName), "%s%s%s%s",m_szTagColor,m_szNameTag,m_szNameColor, playerName);
	}
	else {
		Format(STRING(m_szName), "%s%s%s%s",m_szTagColor,m_szNameTag,m_szNameColor, name);
	}
