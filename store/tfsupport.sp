...
new CPaint1[7]={12073019,4732984 ,11049612 ,3874595 ,6637376 ,8400928 ,12807213 };
new CPaint2[7]={5801378, 3686984,8626083,1581885,2636109,2452877,12091445 };
...
stock GiveAttHat(client, Data)
{
	...
	while((hat=FindEntityByClassname(hat, "tf_wearable"))!=INVALID_ENT_REFERENCE){
		if(GetEntPropEnt(hat, Prop_Send, "m_hOwnerEntity") == client){
			if (!IsValidEntity(hat)) continue;
			
			int iItemDefinitionIndex = GetEntProp(hat, Prop_Send, "m_iItemDefinitionIndex");
			if(FindValueInArray(g_hHatIDs, iItemDefinitionIndex)==-1)continue;
			
			PrintToChat(client,"%d",iItemDefinitionIndex);
			TF2Attrib_SetByDefIndex(hat, 134, float(g_eTFUnusual[Data]));
...
public Action:TF2Items_OnGiveNamedItem(client, String:classname[], iItemDefinitionIndex, &Handle:hItem)
{
	...
	if(m_iEquippedDye >= 0)
	{
		...
		switch(g_eTFHatDye[m_iDataDye][0])
		{
			case 1: {
				m_iDyeId++;
				TF2Items_SetNumAttributes(hItem, 2);
				new DataPaint=g_eTFHatDye[m_iDataDye][1];
				TF2Items_SetAttribute(hItem, m_iDyeId-1, 142, float(CPaint1[DataPaint]));
				TF2Items_SetAttribute(hItem, m_iDyeId, 261, float(CPaint2[DataPaint]));
			}
			case 2: {
				TF2Items_SetAttribute(hItem, m_iDyeId, 1004, float(g_eTFHatDye[m_iDataDye][1]));
			}
			default: {TF2Items_SetAttribute(hItem, m_iDyeId, 142, float(RgbToDec(g_eTFHatDye[m_iDataDye][0], g_eTFHatDye[m_iDataDye][1], g_eTFHatDye[m_iDataDye][2])));}
		}
	}
	TF2Items_SetNumAttributes(hItem, m_iDyeId+1);
...
public bool:TFSupport_ReadItemSchema()
{
	...
	do
	{
		...
		{
			KvGetString(m_hKV, "equip_region", m_szSlot, sizeof(m_szSlot));
			if(strcmp(m_szSlot, "hat")==0)
				PushArrayCell(g_hHatIDs, StringToInt(m_szItemID));
